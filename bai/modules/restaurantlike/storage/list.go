...
func (s *sqlStore) GetUserLikeRestaurant(ctx context.Context,
	...
	if v := paging.FakeCursor; v != "" {
		timeCreated, err := time.Parse(time.Layout, string(base58.Decode(v)))
		if err != nil {
			return nil, common.ErrDB(err)
		}
	...
	for i, item := range result {
		...
		if i == (len(result) - 1) {
			cursorStr := base58.Encode([]byte(fmt.Sprintf("%v", item.CreatedAt.Format("2006-01-02 15:04:05.999999-07:00"))))
